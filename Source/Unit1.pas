...
const
  ...
  NET_FW_RULE_DIR_IN = 1;   // IN  - incoming connections / âõîäÿùèå ñîåäèíåíèÿ
  NET_FW_RULE_DIR_OUT = 2;  // OUT - outgoing / èñõîäÿùèå
...
var
  ...
  NewRule: OleVariant;
begin
  Profile:=NET_FW_PROFILE2_PRIVATE or NET_FW_PROFILE2_PUBLIC or NET_FW_PROFILE2_DOMAIN; // Ïðîôèëè
  ...
  NewRule.Protocol:=NET_FW_IP_PROTOCOL; // Ïðîòîêîëû
  NewRule.Direction:=NET_FW_RULE_DIR; // incoming connections, outgoing / Âõîäÿùèå è èñõîäÿùèå ñîåäèíåíèÿ
  NewRule.Enabled:=true;
  NewRule.Grouping:=AppID;
  NewRule.Profiles:=Profile;
  NewRule.Action:=NET_FW_ACTION_BLOCK; // NET_FW_ACTION_BLOCK - çàïðåòèòü, NET_FW_ACTION_ALLOW - ðàçðåøèòü
...
begin
  ...
  // Add all rules to Firewall / Äîáàâëÿåì âñå ïðàâèëà â Firewall
  ...
  // Update the list, update RuleNames, RulePaths / Îáíîâëÿåì ñïèñîê, îáíîâëÿåì RuleNames, RulePaths
  Main.LoadRegRules;
end;
...
function GetLocaleInformation(Flag: integer): string; // If there are multiple languages in the system (with sorting) / Åñëè â ñèñòåìå íåñêîëüêî ÿçûêîâ (ñ ñîðòèðîâêîé)
var
...
begin
  if ListView.ItemIndex <> - 1 then begin
    Status(Format(ID_RULE_SUCCESSFULLY_REMOVED, [CutStr(ExtractFileName(RulePaths.Strings[ListView.ItemIndex]), 22)])); // Ïîñëå óäàëåíèÿ íàçâàíèÿ óæå íå áóäåò, ïîýòîìó ïåðåä óäàëåíèåì
...
procedure TMain.ListViewKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  // Fixing the bug that hides controls / Óáèðàåì áàã ñêðûòèÿ êîíòðîëîâ
  if Key = VK_MENU then
    Key:=0;
end;
